...
public class AlluxioWriter extends Writer {
...
//        private HashSet<String> endFiles = new HashSet<String>();//最终文件全路径
        ...
        private AlluxioHelper alluxioHelper = null;
        ...
        public void init() {
            ...
            //创建textfile存储
            alluxioHelper = new AlluxioHelper();

            alluxioHelper.getFileSystem(masterHostName, this.writerSliceConfig);
        }

        private void validateParameter() {
            this.masterHostName = this.writerSliceConfig.getNecessaryValue(Key.MASTER_HOSTNAME, AlluxioWriterErrorCode.REQUIRED_VALUE);
            //fileType check
            this.fileType = this.writerSliceConfig.getNecessaryValue(Key.FILE_TYPE, AlluxioWriterErrorCode.REQUIRED_VALUE);
            if( !fileType.equalsIgnoreCase("ORC") && !fileType.equalsIgnoreCase("TEXT")){
                String message = "AlluxioWriter插件目前只支持ORC和TEXT两种格式的文件,请将filetype选项的值配置为ORC或者TEXT";
                throw DataXException.asDataXException(AlluxioWriterErrorCode.ILLEGAL_VALUE, message);
            }
            //path
            this.path = this.writerSliceConfig.getNecessaryValue(Key.PATH, AlluxioWriterErrorCode.REQUIRED_VALUE);
            if(!path.startsWith("/")){
            ...
            //fileName
            this.fileName = this.writerSliceConfig.getNecessaryValue(Key.FILE_NAME, AlluxioWriterErrorCode.REQUIRED_VALUE);
            //columns check
            this.columns = this.writerSliceConfig.getListConfiguration(Key.COLUMN);
            if (null == columns || columns.size() == 0) {
                throw DataXException.asDataXException(AlluxioWriterErrorCode.REQUIRED_VALUE, "您需要指定 columns");
            }else{
                for (Configuration eachColumnConf : columns) {
                    eachColumnConf.getNecessaryValue(Key.NAME, AlluxioWriterErrorCode.COLUMN_REQUIRED_VALUE);
                    eachColumnConf.getNecessaryValue(Key.TYPE, AlluxioWriterErrorCode.COLUMN_REQUIRED_VALUE);
                }
            }
            //writeMode check
            this.writeMode = this.writerSliceConfig.getNecessaryValue(Key.WRITE_MODE, AlluxioWriterErrorCode.REQUIRED_VALUE);
            writeMode = writeMode.toLowerCase().trim();
            Set<String> supportedWriteModes = Sets.newHashSet("append", "truncate");
            if (!supportedWriteModes.contains(writeMode)) {
                throw DataXException.asDataXException(AlluxioWriterErrorCode.ILLEGAL_VALUE,
                        String.format("仅支持append, truncate, 不支持您配置的 writeMode 模式 : [%s]",
                                writeMode));
            }
            ...
            if(null == fieldDelimiter){
                throw DataXException.asDataXException(AlluxioWriterErrorCode.REQUIRED_VALUE,
            ...
            if(haveKerberos) {
                this.writerSliceConfig.getNecessaryValue(Key.KERBEROS_KEYTAB_FILE_PATH, AlluxioWriterErrorCode.REQUIRED_VALUE);
                this.writerSliceConfig.getNecessaryValue(Key.KERBEROS_PRINCIPAL, AlluxioWriterErrorCode.REQUIRED_VALUE);
            }
            // encoding check
        ...
        public void prepare() {
            //若路径已经存在，检查path是否是目录
            if(alluxioHelper.isPathExists(path)){
//                if(!alluxioHelper.isPathDir(path)){
//                    throw DataXException.asDataXException(AlluxioWriterErrorCode.ILLEGAL_VALUE,
//                            String.format("您配置的path: [%s] 不是一个合法的目录, 请您注意文件重名, 不合法目录名等情况.",
//                                    path));
//                }
                //根据writeMode对目录下文件进行处理
                AlluxioURI[] existFilePaths = alluxioHelper.alluxioDirList(path,fileName);
                LOG.info("file count of path:{} is:{}",path,existFilePaths.length);
                boolean isExistFile = false;
                if(existFilePaths.length > 0){
                    isExistFile = true;
                }
                LOG.info("isExistFile:{}",isExistFile);
                ...
                if ("truncate".equalsIgnoreCase(writeMode) && isExistFile ) {
                     ...
                     alluxioHelper.deleteFiles(existFilePaths);
                }
...
//                    for (AlluxioURI eachFile : existFilePaths) {
...
//                }
        ...
        @Override
        public void post() {
//            alluxioHelper.renameFile(tmpFiles, endFiles);
        }

        @Override
        public void destroy() {
            alluxioHelper.closeFileSystem();
        }
        ...
        public List<Configuration> split(int mandatoryNumber) {
            ...
            //获取该路径下的所有已有文件列表
            if(alluxioHelper.isPathExists(path)){
                allFiles.addAll(Arrays.asList(alluxioHelper.alluxioDirList(path)));
            }
            ...
            for (int i = 0; i < mandatoryNumber; i++) {
                ...
                fullFileName = String.format("%s/%s__%s", path, filePrefix, fileSuffix);
                endFullFileName = String.format("%s/%s__%s", path, filePrefix, fileSuffix);
...
//            }
//            while(alluxioHelper.isPathExists(tmpFilePath)){
//                tmpSuffix = UUID.randomUUID().toString().replace('-', '_');
//                if (!isEndWithSeparator) {
//                    tmpFilePath = String.format("%s__%s%s", userPath, tmpSuffix, IOUtils.DIR_SEPARATOR);
//                }else if("/".equals(userPath)){
...
//        }
    ...
    public static class Task extends Writer.Task {
        ...
        public void init() {
            ...
            this.masterHostName = this.writerSliceConfig.getString(Key.MASTER_HOSTNAME);
            ...
            alluxioHelper = new AlluxioHelper();
            alluxioHelper.getFileSystem(masterHostName, writerSliceConfig);
        }
        ...
        public void startWrite(RecordReceiver lineReceiver) {
            ...
            if(fileType.equalsIgnoreCase("TEXT")){
                //写TEXT FILE
                alluxioHelper.textFileStartWrite(lineReceiver,this.writerSliceConfig, this.fileName,
                        this.getTaskPluginCollector());
            }else if(fileType.equalsIgnoreCase("ORC")){
                //写ORC FILE
//                alluxioHelper.orcFileStartWrite(lineReceiver,this.writerSliceConfig, this.fileName,
//                        this.getTaskPluginCollector());
            }
